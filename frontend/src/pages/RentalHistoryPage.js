...
import RentReturnModal from '../components/RentReturnModal';
...
const RentalHistoryPage = () => {
  ...
  const handleApplyFilters = async (pageNumber, pageSize) => {
    ...
    try {
      await getRentals({ userEmail, rentalFilterData, token, page: pageNumber, size: pageSize });
    } catch (err) {
      ...
      {userEmail && (
        <>
          <Typography variant="h2" mb={4}>
            Rental history of {userEmail} 👁️‍🗨️
          </Typography>
                          ...
                          <CardContent sx={{ flex: '1 0 auto' }}>
                            ...
                            >
                              <Button
                                variant="outlined"
                                color="secondary"
                                disabled={rental.rentalStatus !== 'OUT'}
          ...
          {selectedRental && (
            <RentReturnModal rental={selectedRental} open={modalOpen} handleClose={handleCloseModal} />
          )}
        </>
      )}
    </Container>
